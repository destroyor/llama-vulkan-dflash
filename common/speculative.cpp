...
struct common_speculative_state_dflash : public common_speculative_state {
    ...
    int get_adaptive_n_max(int n_max) {
        if (adaptive_n_max == 0) {
            adaptive_n_max = std::min(n_max, 5);
        }
    ...
    int build_cross_data(llama_context * ctx) {
        ...
        for (int t = 0; t < cross_len; ++t) {
            ...
            for (int layer = 0; layer < n_target_layers; ++layer) {
                memcpy(&cross_buf[(size_t)t * n_target_features + (size_t)layer * n_embd],
    ...
    void accept(uint16_t n_accepted) override {
        ...
        if (n_draft_last > 0) {
            ...
            if (accept_rate_ema < DISABLE_ACCEPT_THRESHOLD && n_low_streak >= LOW_STREAK_THRESHOLD) {
                disabled = true;
                cooldown_tokens = 0;
