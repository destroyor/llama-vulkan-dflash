...
float16_t dequantFuncTQ3_0(const in decodeBufTQ3_0 bl, const in uint blockCoords[2], const in uint coordInBlock[2])
{
    const float16_t d = bl.block.d;
    const uint idx = coordInBlock[1];
    const uint group = idx / 8u;
    const uint within = idx % 8u;
    const uint base = group * 3u;
    uint b0 = uint(unpack8(bl.block.qs[base / 2])[base & 1]);
    uint b1 = uint(unpack8(bl.block.qs[(base + 1) / 2])[(base + 1) & 1]);
    uint b2 = uint(unpack8(bl.block.qs[(base + 2) / 2])[(base + 2) & 1]);
...
layout(buffer_reference, std430, buffer_reference_align = 2) buffer decodeBufTURBO4_0 {
   block_turbo4_0_packed16 block;
};

const float16_t turbo4_centroids_cm2[16] = {
    float16_t(-0.241556), float16_t(-0.182907), float16_t(-0.143047), float16_t(-0.111065),
    float16_t(-0.083317), float16_t(-0.058069), float16_t(-0.034311), float16_t(-0.011353),
    float16_t( 0.011353), float16_t( 0.034311), float16_t( 0.058069), float16_t( 0.083317),
    float16_t( 0.111065), float16_t( 0.143047), float16_t( 0.182907), float16_t( 0.241556)
};
...
float16_t dequantFuncTURBO4_0(const in decodeBufTURBO4_0 bl, const in uint blockCoords[2], const in uint coordInBlock[2])
{
    const float16_t d = bl.block.d;
    const uint idx = coordInBlock[1];
    const uint byte_idx = idx / 2u;
    const uint byte_half = idx % 2u;
    uint8_t byte_val = unpack8(bl.block.qs[byte_idx / 2])[byte_idx & 1];
    uint ci = byte_half == 0u ? uint(byte_val & 0xFu) : uint(byte_val >> 4);
    return turbo4_centroids_cm2[ci] * d;
}
