...
FLOAT_TYPEV4 dequantize4(uint ib, uint iqs, uint a_offset, uint binding_idx) {
    ...
    uint b0, b1, b2;
    if (binding_idx == BINDING_IDX_K) {
        b0 = uint(unpack8(k_packed.k_data_packed16[a_offset + ib].qs[base / 2])[base & 1]);
        b1 = uint(unpack8(k_packed.k_data_packed16[a_offset + ib].qs[(base + 1) / 2])[(base + 1) & 1]);
        b2 = uint(unpack8(k_packed.k_data_packed16[a_offset + ib].qs[(base + 2) / 2])[(base + 2) & 1]);
    } else {
        b0 = uint(unpack8(v_packed.v_data_packed16[a_offset + ib].qs[base / 2])[base & 1]);
        b1 = uint(unpack8(v_packed.v_data_packed16[a_offset + ib].qs[(base + 1) / 2])[(base + 1) & 1]);
        b2 = uint(unpack8(v_packed.v_data_packed16[a_offset + ib].qs[(base + 2) / 2])[(base + 2) & 1]);
    }
    uint bits24 = b0 | (b1 << 8) | (b2 << 16);
    uint shift = within * 3u;
    uint idx0 = (bits24 >> shift) & 7u;
    uint idx1 = (bits24 >> (shift + 3)) & 7u;
    uint idx2 = (bits24 >> (shift + 6)) & 7u;
    uint idx3 = (bits24 >> (shift + 9)) & 7u;
    return scale * FLOAT_TYPEV4(tq3_centroids_fa[idx0], tq3_centroids_fa[idx1], tq3_centroids_fa[idx2], tq3_centroids_fa[idx3]);
}
#endif

#if defined(DATA_A_TURBO4_0)
#define BLOCK_BYTE_SIZE 66
const FLOAT_TYPE turbo4_centroids_fa[16] = FLOAT_TYPE[16](
    FLOAT_TYPE(-0.241556), FLOAT_TYPE(-0.182907), FLOAT_TYPE(-0.143047), FLOAT_TYPE(-0.111065),
    FLOAT_TYPE(-0.083317), FLOAT_TYPE(-0.058069), FLOAT_TYPE(-0.034311), FLOAT_TYPE(-0.011353),
    FLOAT_TYPE( 0.011353), FLOAT_TYPE( 0.034311), FLOAT_TYPE( 0.058069), FLOAT_TYPE( 0.083317),
    FLOAT_TYPE( 0.111065), FLOAT_TYPE( 0.143047), FLOAT_TYPE( 0.182907), FLOAT_TYPE( 0.241556)
);
FLOAT_TYPEV4 dequantize4(uint ib, uint iqs, uint a_offset, uint binding_idx) {
    FLOAT_TYPE scale;
    if (binding_idx == BINDING_IDX_K) {
    ...
    uint8_t b0, b1, b2, b3;
    if (binding_idx == BINDING_IDX_K) {
        b0 = unpack8(k_packed.k_data_packed16[a_offset + ib].qs[iqs / 4])[iqs % 2];
        uint iqs1 = iqs + 1;
        b1 = unpack8(k_packed.k_data_packed16[a_offset + ib].qs[iqs1 / 4])[iqs1 % 2];
        uint iqs2 = iqs + 2;
        b2 = unpack8(k_packed.k_data_packed16[a_offset + ib].qs[iqs2 / 4])[iqs2 % 2];
        uint iqs3 = iqs + 3;
        b3 = unpack8(k_packed.k_data_packed16[a_offset + ib].qs[iqs3 / 4])[iqs3 % 2];
    } else {
        b0 = unpack8(v_packed.v_data_packed16[a_offset + ib].qs[iqs / 4])[iqs % 2];
        uint iqs1 = iqs + 1;
        b1 = unpack8(v_packed.v_data_packed16[a_offset + ib].qs[iqs1 / 4])[iqs1 % 2];
        uint iqs2 = iqs + 2;
        b2 = unpack8(v_packed.v_data_packed16[a_offset + ib].qs[iqs2 / 4])[iqs2 % 2];
        uint iqs3 = iqs + 3;
        b3 = unpack8(v_packed.v_data_packed16[a_offset + ib].qs[iqs3 / 4])[iqs3 % 2];
    }
    idx0 = (iqs & 1u) == 0u ? uint(b0 & 0xFu) : uint(b0 >> 4);
    idx1 = ((iqs+1) & 1u) == 0u ? uint(b1 & 0xFu) : uint(b1 >> 4);
    idx2 = ((iqs+2) & 1u) == 0u ? uint(b2 & 0xFu) : uint(b2 >> 4);
    idx3 = ((iqs+3) & 1u) == 0u ? uint(b3 & 0xFu) : uint(b3 >> 4);
    return scale * FLOAT_TYPEV4(turbo4_centroids_fa[idx0], turbo4_centroids_fa[idx1], turbo4_centroids_fa[idx2], turbo4_centroids_fa[idx3]);
}
#endif
