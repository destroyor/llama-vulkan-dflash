...
void process_shaders() {
    ...
    for (const bool& fp16 : {false, true}) {
        ...
        for (const bool& f16acc : {false, true}) {
            ...
            for (const auto& tname : type_names) {
                ...
                } else if (tname == "q4_0" || tname == "q8_0" || tname == "tq3_0" || tname == "turbo4_0" || tname == "f32") {
                    std::string data_a_key = "DATA_A_" + to_uppercase(tname);
                    string_to_spv("flash_attn_f32_f16_" + tname, "flash_attn_cm1.comp",
    ...
    for (std::string t : {"q4_0", "q4_1", "q5_0", "q5_1", "q8_0", "iq4_nl", "tq3_0", "turbo4_0"}) {
    ...
    for (std::string t : {"f32", "f16", "bf16", "q4_0", "q4_1", "q5_0", "q5_1", "q8_0", "iq4_nl", "tq3_0", "turbo4_0"}) {
    ...
    string_to_spv("argmax_ext_f32", "argmax_ext.comp", merge_maps(base_dict, {{"A_TYPE", "float"}, {"D_TYPE", "int"}}));
    string_to_spv("topk_ext_f32", "topk_ext.comp", merge_maps(base_dict, {{"A_TYPE", "float"}, {"D_TYPE", "int"}}));
    string_to_spv("cross_ring_interleave_f32", "cross_ring_interleave.comp", merge_maps(base_dict, {{"FLOAT_TYPE", "float"}}));
