...
uint tq3_extract_index(uint ib, uint val_pos, uint a_offset) {
    ...
    uint base = group * 3u;
    uint b0 = uint(data_a[a_offset + ib].qs[base + 0]);
    uint b1 = uint(data_a[a_offset + ib].qs[base + 1]);
    uint b2 = uint(data_a[a_offset + ib].qs[base + 2]);
    uint idx;
    switch (within) {
        ...
        case 4: idx = ((b1 >> 4) | (b2 << 4)) & 7u; break;
        case 5: idx =  (b2 >> 0) & 7u; break;
        case 6: idx =  (b2 >> 3) & 7u; break;
        case 7: idx =  (b2 >> 6) & 7u; break;
    }
    return idx;
}
vec2 dequantize(uint ib, uint iqs, uint a_offset) {
    float d = float(data_a[a_offset + ib].d);
    uint idx0 = tq3_extract_index(ib, iqs + 0, a_offset);
    uint idx1 = tq3_extract_index(ib, iqs + 1, a_offset);
    return d * vec2(tq3_centroids_df[idx0], tq3_centroids_df[idx1]);
}
vec4 dequantize4(uint ib, uint iqs, uint a_offset) {
    float d = float(data_a[a_offset + ib].d);
    uint idx0 = tq3_extract_index(ib, iqs + 0, a_offset);
    uint idx1 = tq3_extract_index(ib, iqs + 1, a_offset);
    uint idx2 = tq3_extract_index(ib, iqs + 2, a_offset);
    uint idx3 = tq3_extract_index(ib, iqs + 3, a_offset);
    return d * vec4(tq3_centroids_df[idx0], tq3_centroids_df[idx1], tq3_centroids_df[idx2], tq3_centroids_df[idx3]);
}
#endif

#if defined(DATA_A_TURBO4_0)
const float turbo4_centroids_df[16] = float[16](
...
vec2 dequantize(uint ib, uint iqs, uint a_offset) {
    float d = float(data_a[a_offset + ib].d);
    uint byte_idx = iqs / 2;
    uint8_t byte_val = data_a[a_offset + ib].qs[byte_idx];
    uint idx0 = uint(iqs & 1u) == 0u ? uint(byte_val & 0xFu) : uint(byte_val >> 4);
    uint iqs1 = iqs + 1;
    uint byte_idx1 = iqs1 / 2;
    uint8_t byte_val1 = data_a[a_offset + ib].qs[byte_idx1];
    uint idx1 = uint(iqs1 & 1u) == 0u ? uint(byte_val1 & 0xFu) : uint(byte_val1 >> 4);
    return d * vec2(turbo4_centroids_df[idx0], turbo4_centroids_df[idx1]);
}
vec4 dequantize4(uint ib, uint iqs, uint a_offset) {
    float d = float(data_a[a_offset + ib].d);
    ...
    uint byte_idx = iqs / 2;
    uint8_t byte_val = data_a[a_offset + ib].qs[byte_idx];
    idx0 = uint(iqs & 1u) == 0u ? uint(byte_val & 0xFu) : uint(byte_val >> 4);
    uint iqs1 = iqs + 1;
    uint byte_idx1 = iqs1 / 2;
    uint8_t byte_val1 = data_a[a_offset + ib].qs[byte_idx1];
    idx1 = uint(iqs1 & 1u) == 0u ? uint(byte_val1 & 0xFu) : uint(byte_val1 >> 4);
    uint iqs2 = iqs + 2;
    uint byte_idx2 = iqs2 / 2;
    uint8_t byte_val2 = data_a[a_offset + ib].qs[byte_idx2];
    idx2 = uint(iqs2 & 1u) == 0u ? uint(byte_val2 & 0xFu) : uint(byte_val2 >> 4);
    uint iqs3 = iqs + 3;
    uint byte_idx3 = iqs3 / 2;
    uint8_t byte_val3 = data_a[a_offset + ib].qs[byte_idx3];
    idx3 = uint(iqs3 & 1u) == 0u ? uint(byte_val3 & 0xFu) : uint(byte_val3 >> 4);
    return d * vec4(turbo4_centroids_df[idx0], turbo4_centroids_df[idx1], turbo4_centroids_df[idx2], turbo4_centroids_df[idx3]);
}
#endif
...
#if defined(DATA_A_Q4_0) || defined(DATA_A_Q5_0) || defined(DATA_A_Q8_0) || defined(DATA_A_IQ1_S) || defined(DATA_A_IQ2_XXS) || defined(DATA_A_IQ2_XS) || defined(DATA_A_IQ2_S) || defined(DATA_A_IQ3_XXS) || defined(DATA_A_IQ3_S) || defined(DATA_A_IQ4_XS) || defined(DATA_A_IQ4_NL) || defined(DATA_A_TQ3_0) || defined(DATA_A_TURBO4_0)
vec2 get_dm(uint ib, uint a_offset) {
