...
static void ggml_compute_forward_ssm_conv_tree_f32(
    ...
    const ggml_tensor * src0 = dst->src[0]; // conv_x
    const ggml_tensor * src1 = dst->src[1]; // conv1d.weight
    const ggml_tensor * src2 = dst->src[2]; // parent_ids
        ...
        for (int i2 = 0; i2 < n_t; ++i2) {
            ...
            for (int i1 = 0; i1 < ir; ++i1) {
                ...
                for (int i0 = 0; i0 < nc; ++i0) {
                    ...
                    sumf += s[slot + i1*ncs] * c[i0 + i1*nc];
                }
                x[i1] = ggml_silu_f32(sumf);
            }
        }
    }
}
...
static void ggml_compute_forward_gated_delta_net_tree_one_chunk(
    ...
    const ggml_fp16_t * inter_base = (const ggml_fp16_t *)src_inter->data;
    ...
    for (int64_t ir = ir0; ir < ir1; ++ir) {
        ...
        const ggml_fp16_t * inter_seq_base = inter_base + iv3 * n_tokens * inter_stride_token;
        ...
        for (int64_t t = 0; t < n_tokens; t++) {
            if (t > 0) {
                ...
                if (parent_t == GDN_TREE_ROOT_PARENT) {
                    memcpy(s_out, s_in, S_v * S_v * sizeof(float));
                } else if (parent_t != (int)(t - 1)) {
                    const ggml_fp16_t * parent_inter = inter_seq_base + parent_t * inter_stride_token + iv1 * S_v * S_v;
                    for (int64_t j = 0; j < S_v * S_v; j++) {
                        s_out[j] = ggml_fp16_to_fp32(parent_inter[j]);
                    }
                }
            }
            ...
            ggml_fp16_t * inter_t = (ggml_fp16_t *)inter_seq_base + t * inter_stride_token + iv1 * S_v * S_v;
            for (int64_t j = 0; j < S_v * S_v; j++) {
                inter_t[j] = ggml_fp32_to_fp16(s_out[j]);
            }
